...
namespace Poilink
{
    public static partial class PoilinkSDK
    {
        ...
        private struct WebPortalOptionsNative
        {
            ...
            public IntPtr onRewardsReceive;
        }
...
#pragma warning restore IDE0051
        ...
        [StructLayout(LayoutKind.Sequential)]
        private struct RewardGrantNative
        {
            public IntPtr grantId;
            public IntPtr itemCode;
            public int quantity;
            public IntPtr missionCode;
        }

        [UnmanagedFunctionPointer(CallingConvention.Cdecl)]
        private delegate void RewardsReceiveCallbackNative(IntPtr grantsPtr, int count);
        ...
        private static RewardsReceiveCallbackNative _showWebPortalOnRewardsReceiveNativeCallback;
#endif
        ...
        private static void OnShowWebPortalClose()
        {
            ...
            _showWebPortalOnRewardsReceiveNativeCallback = null;
#endif
        ...
        [MonoPInvokeCallback(typeof(RewardsReceiveCallbackNative))]
        private static void OnShowWebPortalRewardsReceive(IntPtr grantsPtr, int count)
        {
            var callback = _showWebPortalOnRewardReceiveCallback;
            if (callback == null || grantsPtr == IntPtr.Zero || count <= 0) return;

            var grantIds = new string[count];
            var itemCodes = new string[count];
            var quantities = new int[count];
            var missionCodes = new string[count];
            int stride = Marshal.SizeOf<RewardGrantNative>();
            for (int i = 0; i < count; i++)
            {
                IntPtr itemPtr = new IntPtr(grantsPtr.ToInt64() + (long)i * stride);
                var g = Marshal.PtrToStructure<RewardGrantNative>(itemPtr);
                grantIds[i] = PtrToStringUtf8(g.grantId) ?? string.Empty;
                itemCodes[i] = PtrToStringUtf8(g.itemCode) ?? string.Empty;
                quantities[i] = g.quantity;
                missionCodes[i] = PtrToStringUtf8(g.missionCode) ?? string.Empty;
            }
            ...
            UnityMainThreadDispatcher.Enqueue(() =>
            {
                ...
                for (int i = 0; i < count; i++)
                {
                    callback(grantIds[i], itemCodes[i], quantities[i], missionCodes[i]);
                    if (!string.IsNullOrEmpty(grantIds[i])) ids.Add(grantIds[i]);
                }
                AutoMarkRewardsConsumed(ids);
            });
        }
...
#pragma warning restore IDE0051
        ...
        private static void AutoMarkRewardConsumed(string grantId)
        {
            ...
            AutoMarkRewardsConsumed(new List<string> { grantId });
        }

        private static void AutoMarkRewardsConsumed(List<string> grantIds)
        {
            if (grantIds == null || grantIds.Count == 0) return;
            MarkRewardsChunk(grantIds, 0);
        }

        private static void MarkRewardsChunk(List<string> grantIds, int offset)
        {
            if (offset >= grantIds.Count) return;
            int size = Math.Min(ItemGrantMaxBatchSize, grantIds.Count - offset);
            _InternalMarkItemGrantsConsumed(grantIds.GetRange(offset, size).ToArray(),
                _ => MarkRewardsChunk(grantIds, offset + size),
                (_, __) => MarkRewardsChunk(grantIds, offset + size));
        }
        ...
        private static void _InternalShowWebPortal(WebPortalOptions options)
        {
            ...
            if (options?.OnRewardReceive != null)
            {
                ...
                _showWebPortalOnRewardsReceiveNativeCallback = new RewardsReceiveCallbackNative(OnShowWebPortalRewardsReceive);
                nativeOptions.onRewardsReceive = Marshal.GetFunctionPointerForDelegate(_showWebPortalOnRewardsReceiveNativeCallback);
            }
            else
            {
                _showWebPortalOnRewardReceiveNativeCallback = null;
                ...
                _showWebPortalOnRewardsReceiveNativeCallback = null;
                nativeOptions.onRewardsReceive = IntPtr.Zero;
            }
